...
### Task 15: Human-confirmed per-PR/per-author backfill (June-July gap beyond trailer detection)

Canh confirmed (2026-07-08) that trailer-based detection undercounts real AI usage — most Copilot autocomplete and web-chat-drafted code never produces a git co-author trailer. He initially asked to mark large/complex PRs as AI-assisted by inference; **declined** that (size correlates with many things besides AI, and it would fabricate specific per-PR/per-ticket dollar figures with no defensible evidence). Agreed instead to go PR-by-PR with Canh directly confirming real AI usage from firsthand knowledge — legitimate, since it's specific attributable information from someone who knows the team, not a proxy/guess.

- [x] Pulled the list of June-July merged PRs without `ai-assisted` (287 total, 284 after excluding the 3 same-day onboarding chore PRs #4094-4096).
- [x] Reviewed PR #4093 first — Canh confirmed AI-assisted, then generalized: **all PRs by `hoanng2201`** are AI-assisted (a specific, named-author attestation, not a blanket "everyone always" claim).
- [x] Applied the same backfill mechanics (label, PR body section, `update_ticket` Jira sync) to all 22 of `hoanng2201`'s unlabeled June-July PRs (#3903, 3917, 3921, 3935, 3945, 3946, 3970, 3989, 4011, 4031, 4054, 4062, 4064, 4070, 4071, 4074, 4075, 4080, 4085, 4089, 4090, 4093) via a one-off script — 0 errors, though 6 of them (release/sync PRs authored by hoanng2201: #4064, 4074, 4080, 4085, 4089, 4090) had no resolvable Jira key, so their hours are labeled on GitHub but not recorded in Jira (same limitation as the original backfill, not new).
- [x] Body/PR-description marker distinguishes this batch from the original trailer-based one: `<!-- ai-usage-backfill-human-confirmed-2026-07-08 -->`, with an explicit note that the hours are still a diff-size estimate (not measured) but the *usage itself* is human-confirmed, not inferred.
- [ ] Re-dispatched June + July collection to reflect the new labels — in progress as of this edit.
- [ ] Still going PR-by-PR with Canh for the remaining ~262 unlabeled June-July PRs (not authored by hoanng2201) — continue that process; do not extrapolate further author-level blanket claims without Canh explicitly confirming each one, same as `hoanng2201`.
