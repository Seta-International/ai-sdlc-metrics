...
**Goal:** Build the conversational agent panel — the right-side sliding panel that users toggle via the Bot button in GlobalNav. Includes chat messages, tool call traces, context pills, and message input.

**Architecture:** React components inside `packages/agent/`. The panel reads state from `AgentStateProvider` (Plan 01) and communicates with the backend via tRPC mutations (send message) and tRPC SSE subscriptions (streaming responses, insight push). All components follow the project's design system (`DESIGN.md`).

**Tech Stack:** React 19, TypeScript, Vitest, `@future/api-client` (tRPC), `@future/ui` components
...
**Depends On:** Plan 01 (types, providers, hooks), Plan 02 (backend tRPC routes)
