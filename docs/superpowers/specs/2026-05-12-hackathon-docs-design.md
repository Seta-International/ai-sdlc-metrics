...
4. The docs describe the framework as it stands — the full `@seta/agent-*` surface, the agent product example, the channels, and the apps/api host are all implemented and stable. No "coming soon" sections, no aspirational APIs, no fallbacks for unmerged code.
...
- **Two install paths in v1.** An in-monorepo path (primary, simplest — clone, scaffold a product, run) and an external-repo path (consume `@seta/agent-*` from a fresh repo). The external-repo path requires the publishing mechanism (npm public vs GitHub Packages vs git-URL/tarball) to be chosen before the page is finalized — see Risks.
- **No web admin UI for hackathon teams.** Studio is not part of the hackathon surface. Demo surface is the team's own minimal chat page over HTTP/SSE.
...
These are all hackathon-process or org-decision questions, not framework-implementation gaps. The framework itself is treated as fully implemented and stable for the purposes of this spec.

1. **External-repo publishing mechanism is undecided.** `external-repo.md` cannot be finalized until we choose one of: (a) flip `@seta/agent-*` packages to public + publish to npm, (b) publish to GitHub Packages with a hackathon-scoped read token, or (c) document a git-URL / tarball install with pinned commit SHAs. Each option has different ops cost; (c) is fastest for a one-off hackathon but worst long-term. Org decision needed before the page is finalized.
2. **External-repo migrations distribution.** Once published, teams need to bring up the kernel + memory + RAG schemas in their own Postgres. Either each package exposes a `migrate` script, or we ship a single SQL bundle. Plan-level decision.
3. **LLM API keys distribution.** The brief implies the OC provides resources but doesn't pin which models or who pays. The quickstart needs a concrete answer. Confirm with organizers.
4. **One-tenant-per-team vs shared dev tenant.** For hackathon scale, a per-team dev tenant id (UUID picked at scaffold time) is simplest; spec assumes that. If organizers want a shared sandbox tenant, the quickstart changes.
5. **Where does the team's repo live?** Each team forks the seta-os repo and works on a branch / their own fork? Or do they get a clean repo carved from this one? Affects the quickstart's first 5 minutes. Confirm.
6. **Round-2 deadline pressure.** v1 of these docs needs to ship before Round 2 starts. The implementation plan sequences the writing tasks against that deadline.
