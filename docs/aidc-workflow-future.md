...
| Review routing | `.github/CODEOWNERS` (maps each code area to its owner) + three risk tiers declared on every PR: **T1** a change inside one feature module, **T2** touches code shared across modules, **T3** core / high-risk areas (identity, DB migrations, CI), which always ship as their own PR | Review depth follows risk automatically: a T1 goes to the module owner, a T3 always goes to the tech lead. |
