...
7. Three agent products under `modules/products/agent`: Planner (read+write, preview/commit), Analytics (chart-card adaptive cards), Seta FAQ (RAG-backed, citation-bearing).
8. `modules/channels/teams` with JWT/JWKS verification + OBO refresh, bound to the Planner product handler.
...
| -- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| D1 | Seven new platform packages: `agent-core`, `-memory`, `-workflows`, `-chunking`, `-embeddings`, `-vector`, `-rag` | `platform/agent/*`                                                                    |
| D2 | MS Graph client + Planner connector                                                                            | `platform/ms-graph`, `modules/connectors/ms365-planner`                               |
| D3 | Three agent products (Planner, Analytics, FAQ)                                                                 | `modules/products/agent/{planner,analytics,faq}`                                       |
| D4 | Teams channel package                                                                                          | `modules/channels/teams`                                                              |
...
| 9.2    | P1    | Planner-Conn | Drizzle schema (`connector_ms365_planner_*`) + migration + `ConnectorDefinition`.    | AG-F2 | P0  | 9.1      | D03 2026-05-13 | D03 2026-05-13 |      1.0 | Schema applied via `pnpm migrate`; manifest exports `requiredScopes` for admin consent.                                            |
| 9.3    | P1    | Planner-Conn | READ endpoints: `listPlans`, `getPlan`, `listTasks`, `getTask`, `searchTasks`.        | AG-F2 | P0  | 9.1, 9.2 | D04 2026-05-14 | D05 2026-05-15 |      2.0 | Each endpoint returns typed result against recorded fixtures; pagination handles >100 items.                                       |
...
##### EP-10 · Planner Agent product (AG-F1 + AG-S, 8 MD, P0)

| WBS ID | Phase | Feature Area | Task Name                                                                            | Role            | Pri | Deps        | Start          | End            | Est (MD) | DoD / Acceptance Criteria                                                                                                          |
| ------ | ----- | ------------ | ------------------------------------------------------------------------------------ | --------------- | --- | ----------- | -------------- | -------------- | -------: | ---------------------------------------------------------------------------------------------------------------------------------- |
| 10.1   | P1    | Planner-Agt  | Agent definition + Planner system prompt + tool registry.                            | AG-F1           | P0  | 1.2         | D03 2026-05-13 | D03 2026-05-13 |      1.0 | Agent definition file in `modules/products/agent/planner/`; agent loadable into kernel.                                            |
| 10.2   | P1    | Planner-Agt  | READ tools: `list_tasks`, `get_task` (search subsumed via filter in `list_tasks`).   | AG-F1           | P0  | 10.1, 9.3   | D06 2026-05-18 | D07 2026-05-19 |      2.0 | Both tools callable from kernel; integration test asserts shape; consent check via `connectorRegistry.requireConsent`.            |
| 10.3   | P1    | Planner-Agt  | WRITE tools: `preview_create_task` + `commit_create_task` with HMAC continuation.    | AG-F1           | P0  | 10.2, 9.4   | D08 2026-05-20 | D10 2026-05-22 |      2.0 | Preview returns continuation token; commit accepts token + idempotency key; replay produces identical Planner state.                |
...
| 10.5   | P1    | Planner-Agt  | Adaptive Card render: task-list card for READ.                                       | AG-F1           | P0  | 10.2        | D08 2026-05-20 | D08 2026-05-20 |      1.0 | Card schema validates; renders correctly in Teams desktop.                                                                          |
| 10.6   | P1    | Planner-Agt  | Adaptive Card: preview card with Confirm/Cancel actions for WRITE.                   | AG-F1           | P0  | 10.3        | D11 2026-05-25 | D11 2026-05-25 |      1.0 | User click on Confirm posts continuation token; Cancel discards.                                                                   |

##### EP-11 · Analytics Agent (AG-F1 + AG-S, 5 MD, P1)

| WBS ID | Phase | Feature Area | Task Name                                                                            | Role            | Pri | Deps        | Start          | End            | Est (MD) | DoD / Acceptance Criteria                                                                                                          |
| ------ | ----- | ------------ | ------------------------------------------------------------------------------------ | --------------- | --- | ----------- | -------------- | -------------- | -------: | ---------------------------------------------------------------------------------------------------------------------------------- |
| 11.1   | P1    | Analytics    | Agent definition + system prompt + tool registry.                                    | AG-F1           | P1  | 1.2         | D11 2026-05-25 | D11 2026-05-25 |      1.0 | Agent loadable into kernel; system prompt biased toward chart-card output format.                                                  |
| 11.2   | P1    | Analytics    | Aggregation tools: `workload_by_assignee`, `tasks_by_status`, `tasks_by_plan`.        | AG-F1           | P1  | 11.1, 9.3   | D12 2026-05-26 | D13 2026-05-27 |      2.0 | Each tool returns aggregated counts grouped per dimension; uses Planner READ only (no live Graph).                                |
| 11.3   | P1    | Analytics    | Chart-card Adaptive Card schema (Vega-Lite spec embedded; bar/grouped-bar only).      | AG-F1           | P1  | 11.2        | D14 2026-05-28 | D14 2026-05-28 |      1.0 | Card validates against Adaptive Card schema; renders bar chart in Teams desktop.                                                   |
| 11.4   | P1    | Analytics    | AG-S review + integration smoke (Teams query → chart card).                          | AG-S            | P1  | 11.3        | D15 2026-05-29 | D15 2026-05-29 |      1.0 | Live demo query "who is overloaded this week" returns a chart card; reviewed for prompt-injection surface.                          |
...
| 13.5   | P1    | Teams-Ch     | Handler `bindPlanner(planner)` mounts Planner product behind the channel.            | FS              | P0  | 13.4, 10.5        | D10 2026-05-22 | D10 2026-05-22 |      0.5 | "summarize my tasks" → Planner agent → task list card in Teams.                                                                    |
...
| 14.3   | P1    | apps/api     | `main.ts` mounts kernel + memory + workflows + Teams channel + Planner product.      | FS              | P0  | 14.1, 14.2, 1.5, 2.2, 3.2, 13.5 | D10 2026-05-22 | D11 2026-05-25 |      1.0 | `pnpm dev` boots; routes mounted at expected prefixes; smoke curl green.                                                            |
...
| EP-09  | MS Graph + Planner connector                            |     6 |    9.0   |
| EP-10  | Planner Agent product                                   |     6 |    8.0   |
...
- Drop order (pre-authorised if utilisation rises above 100% effective): 11.4 chart-card review → 11.3 chart renderer → 7.3 RAG library tests → 3.3 `.parallel()` workflow → 11.2 Analytics aggregation tool count from 3 → 2.
