...
| R-03 | **Capacity over-commit.** Midpoint WBS estimate (~322 MD) exceeds builder capacity (~114 MD over 8 weeks net of VN holidays) by ~2.8×.                                                                       | H   | H   | **H+** | At Kickoff: pick scope-trade path per `03-Scope` Block 1 Recommendation (defer Admin console · compress aggregated views · scope conversational AI to pre-built queries · lean on DE if onboarded). Rebaseline via CR.                                                                                        | PM             | Open   |
...
| ------------- | ---------------------------------------- | -------------------------------- |
| P1 — Critical | Prod-impacting, security, data loss      | 15 min response · 4 h resolve    |
