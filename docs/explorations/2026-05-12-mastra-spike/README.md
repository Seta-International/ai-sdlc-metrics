...
- **§3 (line 117):** Spell out the `write_continuations` row shape: `continuation_id UUID, tenant_id, tool_id, input_hash, etag_snapshot jsonb, hmac, expires_at, consumed_at`; HMAC-SHA-256 over canonicalized payload + server secret from `@seta/auth` KMS. (SA-4)
...
- `Run` identifier (UUID) + `RunStatus` type — even with no workflow primitive. (SA-5)
