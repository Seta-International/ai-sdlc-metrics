...
- setup.md §3: add a `toJSON`/serialization note for forwarding tenant context to background jobs — borrow Mastra's "strip non-serializable" guarantee from `mastra/packages/core/src/request-context/index.ts:168-193` (Idempotent external boundaries, CLAUDE.md).
