...
- **`transformRequest` normalizer** applied on both record and replay paths. We will need this for `tenant_id`, `run_id` (uuid), `timestamp` fields that the kernel injects per request.
