...
| R-07.38a | **Cross-tenant leak canary.** A daily `observability-leak-canary` pg-boss job emits a synthetic fixture-tenant turn with `canary_marker: true` and scans every other tenant's trace-read surface for any match. Non-zero match = P0 security incident; exporter read plane disabled for investigation. Runs regardless of backend choice. **Status: DEFERRED** — the scan requires querying the trace-backend read surface, which is vendor-deferred (§17). The pg-boss job runs on schedule and records `result='deferred'` in `agent_cross_tenant_leak_canary_total` to make the gap visible in dashboards. Activation runbook: when the trace backend adapter is deployed, replace the stub in `leak-canary.scheduler.ts#run()` per the class-level JSDoc. | §12, §18  |
