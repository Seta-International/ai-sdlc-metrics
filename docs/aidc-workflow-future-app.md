...
| Local verification gates | Before any commit or push:<br/>• formatting, lint, type checks<br/>• architecture boundaries<br/>• database rules: migrations, SQL audit, schema drift<br/>• style containment in the shared UI kit<br/>• module shape, test layout, RBAC coverage<br/>Plus **unit and integration tests (TDD)** on a real database. | A machine-enforced quality floor.<br/>A failed check sends the work back to the agent, not to a person. |
