...
-- at least 78 review rows (enhanced data adds more employees/periods)
SELECT cmp_ok( (SELECT count(*) FROM elc.performance_review)::int, '>=', 78, '>=78 review rows' );
SELECT cmp_ok( (SELECT count(DISTINCT report_period) FROM elc.performance_review)::int, '>=', 2,
               '>=2 distinct review periods present' );

-- at least 80 timesheet rows (enhanced data adds more employees/periods)
SELECT cmp_ok( (SELECT count(*) FROM elc.timesheet_monthly)::int, '>=', 80, '>=80 timesheet rows' );
