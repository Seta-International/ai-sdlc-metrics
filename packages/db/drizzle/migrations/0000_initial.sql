...
CREATE TABLE "admin"."tenant_settings" (
	...
	"planner_ms_sync_rosters_enabled" boolean DEFAULT false NOT NULL,
...
CREATE TABLE "planner"."ms_linked_roster" (
	...
	"linked_at" timestamp with time zone DEFAULT now() NOT NULL,
	"sync_enabled" boolean DEFAULT true NOT NULL,
	"minted_by_future_at" timestamp with time zone,
	"unlinked_at" timestamp with time zone
);
--> statement-breakpoint
...
CREATE TABLE "planner"."roster_member" (
	...
	"sso_subject" text NOT NULL,
	"synced_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "roster_member_tenant_id_ms_roster_id_sso_subject_pk" PRIMARY KEY("tenant_id","ms_roster_id","sso_subject")
);
--> statement-breakpoint
...
-- planner.ms_linked_roster
ALTER TABLE planner.ms_linked_roster ENABLE ROW LEVEL SECURITY;
ALTER TABLE planner.ms_linked_roster FORCE ROW LEVEL SECURITY;
CREATE POLICY ms_linked_roster_tenant_isolation
  ON planner.ms_linked_roster
  USING (tenant_id = current_setting('app.tenant_id', true)::uuid)
  WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::uuid);

-- planner.roster_member
ALTER TABLE planner.roster_member ENABLE ROW LEVEL SECURITY;
ALTER TABLE planner.roster_member FORCE ROW LEVEL SECURITY;
CREATE POLICY roster_member_tenant_isolation
