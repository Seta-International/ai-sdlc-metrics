...
> {
  ...
  async execute(query: GetCarryOverCandidatesQuery): Promise<MyDayTask[]> {
    const { actorId, tenantId, today } = query

    if (!DATE_RE.test(today) || Number.isNaN(new Date(`${today}T00:00:00Z`).getTime())) {
      throw new BadRequestException('invalid date: today must be YYYY-MM-DD')
    }

    const yesterday = minusOneDay(today)

    // Query 1: yesterday's my_day_entry rows that are still open (not completed, task not at 100%)
