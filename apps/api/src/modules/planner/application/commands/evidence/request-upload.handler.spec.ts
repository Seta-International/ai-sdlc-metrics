...
import { RequestEvidenceUploadHandler } from './request-upload.handler'
...
describe('RequestEvidenceUploadHandler', () => {
  let handler: RequestEvidenceUploadHandler
  ...
  beforeEach(() => {
    ...
    handler = new RequestEvidenceUploadHandler(
  ...
  it('returns uploadUrl, storageKey, expiresAt for a valid request', async () => {
    const command = new RequestEvidenceUploadCommand(
      TENANT_ID,
      PLAN_ID,
      TASK_ID,
      ACTOR_ID,
    ...
    expect(key).toMatch(new RegExp(`^${TENANT_ID}/documents/planner-evidence/${TASK_ID}/`))
  ...
  it('does NOT create a DB row', async () => {
    const command = new RequestEvidenceUploadCommand(
      TENANT_ID,
      PLAN_ID,
      TASK_ID,
