...
import { ObservabilityPiiRedactionEvaluator } from './observability-pii-redaction.evaluator'
...
describe('ObservabilityPiiRedactionEvaluator', () => {
  it('passes when PII leak count is 0', async () => {
    const metrics = buildMetrics(0)
    const evaluator = new ObservabilityPiiRedactionEvaluator(metrics)
  ...
  it('fails when PII leak count is > 0', async () => {
  ...
  it('has the correct id and section', () => {
    const metrics = buildMetrics(0)
    const evaluator = new ObservabilityPiiRedactionEvaluator(metrics)
    expect(evaluator.id).toBe('18.4.pii_redaction_at_capture')
