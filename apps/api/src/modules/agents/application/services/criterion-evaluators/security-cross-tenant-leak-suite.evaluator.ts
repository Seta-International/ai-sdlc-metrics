...
const CRITERION_ID = '18.2.cross_tenant_leak_suite'
const { threshold } = CRITERION_THRESHOLDS[CRITERION_ID]

/**
 * §18.2 — Cross-Tenant Leak Suite
 *
 * Checks whether the CI cross-tenant leak test suite passed in the window.
 *
 * result = ciState.checkPassed({ checkName: 'cross-tenant-leak-suite', window })
 * passed = result === true
 * unableToEvaluate = result === null
 */
@Injectable()
export class SecurityCrossTenantLeakSuiteEvaluator implements CriterionEvaluator {
