...
import { CostPerTurnVarianceEvaluator } from './cost-per-turn-variance.evaluator'
...
describe('CostPerTurnVarianceEvaluator', () => {
  it('passes when variance <= 0.10', async () => {
    const metrics = buildMetrics(0.08)
    const evaluator = new CostPerTurnVarianceEvaluator(metrics)

    const result = await evaluator.evaluate(WINDOW)

    expect(result.passed).toBe(true)
    expect(result.observedValue).toBe('0.0800')
    expect(result.unableToEvaluate).toBeUndefined()
  })

  it('fails when variance > 0.10', async () => {
    const metrics = buildMetrics(0.15)
    const evaluator = new CostPerTurnVarianceEvaluator(metrics)

    const result = await evaluator.evaluate(WINDOW)

    expect(result.passed).toBe(false)
    expect(result.observedValue).toBe('0.1500')
    expect(result.unableToEvaluate).toBeUndefined()
  })
  ...
  it('has the correct id and section', () => {
    ...
    const evaluator = new CostPerTurnVarianceEvaluator(metrics)
    expect(evaluator.id).toBe('18.3.per_turn_cost_p95_variance_week_over_week')
