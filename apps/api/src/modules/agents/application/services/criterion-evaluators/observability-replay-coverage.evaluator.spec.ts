...
import { ObservabilityReplayCoverageEvaluator } from './observability-replay-coverage.evaluator'
...
describe('ObservabilityReplayCoverageEvaluator', () => {
  it('passes when replay success rate >= 1.0 (100%)', async () => {
    const metrics = buildMetrics(1.0)
    const evaluator = new ObservabilityReplayCoverageEvaluator(metrics)
  ...
  it('fails when replay success rate < 1.0', async () => {
  ...
  it('has the correct id and section', () => {
    ...
    const evaluator = new ObservabilityReplayCoverageEvaluator(metrics)
    expect(evaluator.id).toBe('18.4.replay_coverage_on_100_sampled')
