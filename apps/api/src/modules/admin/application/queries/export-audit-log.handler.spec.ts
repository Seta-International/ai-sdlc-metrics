...
describe('ExportAuditLogHandler', () => {
  ...
  it('properly quotes fields containing commas', async () => {
    const eventWithComma: AuditEventRow = {
      id: '01900000-0000-7000-8000-000000000071',
      tenantId: TENANT_ID,
      actorId: '01900000-0000-7000-8000-000000000005',
      eventType: 'role.granted,revoked', // has comma
      module: 'kernel',
      subjectId: '01900000-0000-7000-8000-000000000050',
      payload: { note: 'test' },
      createdAt: new Date('2026-04-11T10:00:00Z'),
    }
    vi.mocked(auditQueryRepo.query).mockResolvedValue({ items: [eventWithComma], total: 1 })

    const result = await handler.execute(new ExportAuditLogQuery(TENANT_ID))

    // eventType has comma — should be quoted
    expect(result).toContain('"role.granted,revoked"')
    // Result should have exactly 2 lines (header + 1 row)
    expect(result.split('\n').length).toBe(2)
  })
})
