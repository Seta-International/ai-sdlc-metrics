...
describe('PeopleTrpcService', () => {
  let commandBus: { execute: ReturnType<typeof vi.fn> }
  let queryBus: { execute: ReturnType<typeof vi.fn> }
  let service: PeopleTrpcService

  beforeEach(() => {
    commandBus = { execute: vi.fn() }
    queryBus = { execute: vi.fn() }
    service = new PeopleTrpcService(commandBus as never, queryBus as never)
  })
  ...
  describe('getInstance()', () => {
    it('throws if called before onModuleInit', () => {
      ...
      expect(() => PeopleTrpcService.getInstance()).toThrow('PeopleTrpcService not initialized')
    })

    it('returns the singleton after onModuleInit', () => {
      service.onModuleInit()
      const got = PeopleTrpcService.getInstance()
      expect(got).toBe(service)
    })

    it('returns the most recently initialized singleton', () => {
      const service2 = new PeopleTrpcService(commandBus as never, queryBus as never)
      service.onModuleInit()
      service2.onModuleInit()
      expect(PeopleTrpcService.getInstance()).toBe(service2)
    })
  })
  ...
  describe('onModuleInit()', () => {
    it('sets the module-level singleton to the current instance', () => {
      service.onModuleInit()
      expect(PeopleTrpcService.getInstance()).toBe(service)
    })
  })
