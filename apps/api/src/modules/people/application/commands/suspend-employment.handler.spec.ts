...
describe('SuspendEmploymentHandler', () => {
  ...
  it('throws EmploymentNotFoundException when employment does not exist', async () => {
    ...
    await expect(
      handler.execute(
        new SuspendEmploymentCommand(TENANT_ID, EMPLOYMENT_ID, 'reason', new Date(), INITIATED_BY),
      ),
  ...
  it('throws InvalidEmploymentStatusTransitionException when already suspended', async () => {
    vi.mocked(employmentRepo.findById).mockResolvedValue(
