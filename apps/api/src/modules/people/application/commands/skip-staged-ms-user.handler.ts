...
@CommandHandler(SkipStagedMsUserCommand)
export class SkipStagedMsUserHandler implements ICommandHandler<SkipStagedMsUserCommand, void> {
  constructor(
    @Inject(MS_STAGED_USER_REPOSITORY)
    private readonly stagedUserRepo: IMsStagedUserRepository,
  ) {}

  async execute(command: SkipStagedMsUserCommand): Promise<void> {
    ...
    await this.stagedUserRepo.updateStatus(command.stagedUserId, command.tenantId, 'skipped')
  }
}
