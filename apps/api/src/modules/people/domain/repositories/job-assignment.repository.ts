...
export interface IJobAssignmentRepository {
  ...
  findCurrentMany(employmentIds: string[], tenantId: string): Promise<JobAssignment[]>
  findCurrentByManagerId(managerId: string, tenantId: string): Promise<JobAssignment[]>
  countCurrentByManagerId(managerId: string, tenantId: string): Promise<number>
