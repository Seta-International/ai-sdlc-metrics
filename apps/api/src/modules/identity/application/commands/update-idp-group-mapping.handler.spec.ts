...
describe('UpdateIdpGroupMappingHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new UpdateIdpGroupMappingHandler(providerRepo, mappingRepo, auditFacade)
  })
  ...
  it('upserts a group mapping when provider exists', async () => {
    ...
    expect(mappingRepo.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ externalGroupId: 'aad-group-123', roleKey: 'employee' }),
    )
    expect(auditFacade.recordEvent).toHaveBeenCalledWith(
      expect.objectContaining({ eventType: 'idp_group_mapping_updated', module: 'identity' }),
    )
  })
