...
> {
  constructor(
    @Inject(MAGIC_LINK_TOKEN_REPOSITORY) private readonly tokenRepo: IMagicLinkTokenRepository,
  ) {}
  ...
  async execute(command: ValidateMagicLinkCommand): Promise<ValidateMagicLinkResult> {
    ...
    if (!token) throw new MagicLinkTokenNotFoundException()
    await this.tokenRepo.markUsed(token.id, token.tenantId)
    return { email: token.email, tenantId: token.tenantId }
  }
}
