...
describe('CreateSystemActorHandler', () => {
  ...
  it('propagates error when commandBus fails', async () => {
    vi.mocked(commandBus.execute).mockRejectedValue(new Error('Kernel error'))

    await expect(
      handler.execute(new CreateSystemActorCommand(TENANT_ID, 'Bot', ACTOR_ID)),
    ).rejects.toThrow('Kernel error')
  })

  it('propagates error when audit insert fails', async () => {
    vi.mocked(commandBus.execute).mockResolvedValue(NEW_SYSTEM_ACTOR_ID)
    vi.mocked(auditRepo.insert).mockRejectedValue(new Error('DB error'))
