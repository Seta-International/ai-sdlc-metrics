...
export class DrizzleIdpGroupMappingRepository implements IIdpGroupMappingRepository {
  ...
  async findById(id: string, tenantId: string): Promise<IdpGroupMapping | null> {
    const rows = await this.db
      ...
      .limit(1)
    return (rows[0] as IdpGroupMapping | undefined) ?? null
  }
  ...
  async upsert(data: {
    ...
    // The schema defines two partial unique indexes (global/scoped), but the
    // migration applied to the test DB uses a single composite unique index on
    // all 5 columns. We target the full column set for conflict resolution, which
    // works for both partial-index variants.
    const rows = await this.db
