...
- **Allowed internal:** `@seta/agent-core` (`Run` / `RunStatus` types, `KernelError` subclasses), `@seta/db` (pool + `withTenant` + role exports + migration runner), `@seta/tenancy` (context reads — workflows run under the originating tenant), `@seta/audit` (record suspend / resume / step transitions), `@seta/observability` (logger + OTel spans for step boundaries).
