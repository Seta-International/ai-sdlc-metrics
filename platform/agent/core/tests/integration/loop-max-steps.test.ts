...
describe('K4 loop — maxSteps ceiling', () => {
  afterEach(() => recording.stop())

  it.skipIf(!shouldRun('loop-max-steps-anthropic'))(
    'synthesizes finish:length; no tools on the cap-triggering step',
    async () => {
      recording = setupLLMRecording({
        name: 'loop-max-steps-anthropic',
      ...
      const cfg: AgentConfig = {
        ...
        systemPrompt: 'Keep calling the incr tool repeatedly. Never stop on your own.',
        maxTokens: 64,
        tools: [incr],
      }
      ...
      await tenantContext.run({ tenantId: 'tenant-test' }, async () => {
        for await (const c of run(cfg, input, { adapters, maxSteps: 2, ...ctxOverrides })) {
          chunks.push(c)
        }
      })

      expect(chunks.at(-1)).toMatchObject({ type: 'finish', reason: 'length' })
      const loopSpan = exporter.getFinishedSpans().find((s) => s.name === 'agent.run.loop')
      expect(loopSpan?.attributes['loop.stop_reason']).toBe('step_limit')
      expect(loopSpan?.attributes['loop.iterations']).toBe(2)
      expect(toolCount).toBeLessThan(3)
    },
  )
})
